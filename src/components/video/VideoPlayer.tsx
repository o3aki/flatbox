...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ videoSrc }) => {
  ...
  return (
    <div className="relative w-full max-w-[1000px] flex justify-center m-auto group bg-black rounded-xl">
      <div
        className={
          "absolute bottom-0 left-0 right-0 text-white bg-gradient-to-t from-black/40 z-10 opacity-0 transition-opacity group-hover:opacity-100 cursor-pointer"
        }
      >
        <div className="cursor-pointer flex items-center mx-2 h-2 group/timeline">
          <div
            className={
              "w-full relative bg-gray-500 opacity-50 hover:opacity-100 h-1 group-hover/timeline:h-full"
            }
            onClick={handleTimeUpdate}
            ref={timelineRef}
          >
            <span
              style={{ right: `${percentComplete * 100}%` }}
              className="absolute left-0 top-0 bottom-0 bg-[#1EE5AF]"
            ></span>
            <div
              style={{ left: `${100 - percentComplete * 100}%` }}
              className="scale-0 group-hover/timeline:scale-100 absolute h-[200%] aspect-square bg-[#1EE5AF] rounded-full translate-x-[-50%] top-[-50%]"
            ></div>
          </div>
        </div>
          ...
          <div className="flex gap-2 p-3 items-center">
            ...
            <div className="flex items-center gap-1 group/volume">
              ...
              <input
                ...
                className="w-0 h-1 scale-0 group-hover/volume:w-20 group-hover/volume:scale-100 transition-all duration-200 origin-left accent-white"
              />
            </div>
