...
const Sidebar: React.FC<SidebarProps> = ({ subscribedChannels }) => {
  ...
  return (
    <>
      {sidebar?.isOpen && (
        <div
          className={"bg-[#23232F] bg-opacity-50 w-screen h-screen fixed z-30"}
          onClick={() => sidebar.onClose()}
        />
      )}
      <div
        className={`fixed w-64 bg-[#23232F] z-40 mt-2 px-6 flex flex-col h-screen overflow-scroll no-scrollbar ${
